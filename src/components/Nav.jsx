  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-center px-4 max-w-screen-lg mx-auto">
      ...
      >
          ...
          <ul className="flex flex-col md:flex-row items-center md:gap-3 gap-4 py-3 md:pr-6">
            {navItems.map((item) => (
              <Link
                ...
                duration={700}
                key={item.id}
                className="text-[1.05rem] cursor-pointer whitespace-nowrap"
              >
                <li>{item.name}</li>
              </Link>
            ))}
        ...
        <div>
          <button className="px-4 py-3 text-red-900 bg-white tracking-wider border-red-900 border-[1px] rounded-lg hover:bg-red-800 hover:border-white hover:text-white duration-300">
            Reservation
          </button>
        </div>
      </div>
